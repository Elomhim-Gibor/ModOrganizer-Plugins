...
# 如果失败，则回退到 PyQt5
    ...
    # 在 PyQt5 中，StandardButton 可以直接从 QMessageBox 访问
    StandardButton = QMessageBox
# --- 兼容性处理结束 ---

from .utils import check_free_space, get_base_path, get_pagefiles_size, logger, set_pagefile_size

class PageFileManager(mobase.IPluginTool):
    def __init__(self) -> None:
        """
        Initializes a new instance of the PageFileManager class.
        This constructor initializes the base class (mobase.IPluginTool) and sets the internal organizer reference to None.
        ...
        """
        super().__init__()
        self._organizer: mobase.IOrganizer = None  # Internal organizer reference.
        self._need_restart: bool = False           # Flag indicating if a PC restart is needed.
        self._base_path: str = get_base_path()     # Base path of the script.

    def init(self, organizer: mobase.IOrganizer) -> bool:
        """
        Initializes the plugin with the given organizer.
        This function is called by the organizer to initialize the plugin. It sets the internal organizer reference and returns True to indicate successful initialization.
    ...
    def tr(self, txt: str) -> str:
        ...
        This function uses the QCoreApplication.translate function to translate the given text using the context "PageFileManager". The translated text is then returned.
    ...
    def description(self) -> str:
        ...
        This function returns a string that describes what the plugin does. It is used to provide information about the plugin in the user interface.
    ...
    def name(self) -> str:
        ...
        This function returns the name of the plugin as a string. It is used to identify the plugin in the user interface.
    ...
    def localizedName(self) -> str:
        ...
        This function returns the localized name of the plugin as a string. It is used to display the plugin name in the user interface.
    ...
    def requirements(self) -> typing.List[mobase.IPluginRequirement]:
        """
        Returns an empty list of plugin requirements.
        This function is part of the IPluginTool interface and is used to specify the dependencies of the plugin. In this case, the plugin does not have any specific dependencies, so we return an empty list.
    ...
    def version(self) -> mobase.VersionInfo:
        ...
        This function returns a mobase.VersionInfo object with the version information of the plugin. The version information is set to 0.4.0-Beta.
    ...
    def display(self) -> None:
        ...
        current_initial_size_mb: int = 0
        current_max_size_mb: int = 0
        ...
        if len(pagefiles_size) > 0:
            ...
            if pagefile_size[0].lower() == drive_letter:
                current_initial_size_mb = pagefile_size[1]
                current_max_size_mb = pagefile_size[2]
            ...
            if initial_size_mb < required_mb or max_size_mb < maximum_mb:
                ...
                if success:
                    ...
                    # --- 使用兼容的 StandardButton ---
                    result: StandardButton = QMessageBox.question(
                        self._parentWidget(),
                        self.tr("Pagefile is too small."),
                        self.tr(
                            "Page file on all drives is too small. {0}-{1}MB required, but only {2}-{3}MB available. Do you want to automatically set it to {0}-{1}MB?"
                        ).format(required_mb, maximum_mb, initial_size_mb, max_size_mb),
                        StandardButton.Yes | StandardButton.No,
                        StandardButton.No,
                    )
                    if result == StandardButton.Yes:
                        set_pagefile_size(required_mb, maximum_mb, initial_size_mb - current_initial_size_mb, max_size_mb - current_max_size_mb)
            ...
            if check_free_space(required_mb)[0]:
                ...
                if result == StandardButton.Yes:
                    ...
                    set_pagefile_size(required_mb, maximum_mb, 0, 0)
                    self.ask_for_restart()
                    self._need_restart = True
            else:
                QMessageBox.warning(
    ...
    def displayName(self) -> str:
        ...
        This function returns the name of the plugin's display as a string. It is used to display the plugin's name in the Organizer.
    ...
    def icon(self) -> QIcon:
        ...
        This function returns a QIcon object that represents the icon for the plugin. This function is part of the IPluginTool interface and is used to get the icon for the plugin.
    ...
    def tooltip(self) -> str:
        ...
        This function returns a string that contains the tooltip text for the plugin. The tooltip text is used to provide information about what the plugin does.
    ...
    def ask_for_restart(self) -> None:
        ...
        Displays a message box with 'Confirm Restart' and 'Pagefile was set properly. Do you want to restart the computer?'. The user can choose between 'Yes' and 'No'. If 'Yes' is selected, the system will attempt to restart immediately. If 'No' is selected, a message box will be displayed asking the user to restart the system manually later.
        ...
        """
        # --- 使用兼容的 StandardButton ---
        reply: StandardButton = QMessageBox.question(
            self._parentWidget(),
    ...
    def validate_settings(self) -> None:
        ...
        This function ensures that the plugin settings are valid and consistent. If the minimum size is negative, it is set to 20GB. If the minimum size is larger than the maximum size, the maximum size is set to the minimum size.
